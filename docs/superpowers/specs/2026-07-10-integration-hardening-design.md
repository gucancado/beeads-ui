...
- Novo `scripts/gen-classlist.mjs <pkg>`: lê `packages/<pkg>/dist/index.js` + `index.mjs`, extrai todos os literais de string, quebra por whitespace, deduplica e escreve 1 token por linha em `packages/<pkg>/dist/classlist.txt`. Superset é ok: tokens que não são classe (ex.: `beeads_sidebar_collapsed`) são ignorados pelo scanner do Tailwind. Roda no `build` de `ui` e `charts` após o tsup.
...
**A4. Docs + changesets.**
- `docs/CONTRIBUTING.md` (ou README dos pacotes): seção "Integração com Tailwind" com a receita canônica v4 (import dos styles.css já basta; `@source` explícito pro classlist.txt quando o bundler não propagar) e v3 (preset.cjs + `@import vars.css` + `content` incluindo os 2 classlist.txt).
...
- **B4. Trigger mobile:** o ui novo torna a sidebar off-canvas <768px — sem trigger, o mobile perde a navegação. Adicionar no `AppLayout.tsx` uma topbar mobile-only dentro do `<main>`: `<header className="flex items-center gap-2 border-b border-border p-2 md:hidden"><SidebarTrigger /><span className="font-display">bloquim</span></header>`.
- **B5. Verificação local:** `pnpm build` do mindtask-app; **assert no CSS gerado**: `h-svh`, `sticky`, `max-w-\[85vw\]` presentes; app builda sem erro novo de tsc (baseline tem ~71 erros pré-existentes — gate é relativo).
