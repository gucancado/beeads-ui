...
- 5a8deed: Defaults dos gráficos passam a ser o padrão visual da plataforma: grid no token `--color-border` (antes caía no `#ccc` do recharts), ticks dos eixos em `fontSize: 11` (antes 12) e linhas/áreas **lineares** por default (antes `monotone`).
